...
#if 0
        node::WordType determineWordType() const {
            bool hasHiragana = false;
            bool hasKanji = false;
            if (_length == 1 && _baseStr[_begin] == GETA_CHAR) {
                return node::WordType::UNKNOWN;
            }
            for (size_t i = _begin; i < _end; ++i) {
                wchar_t ch = _baseStr[i];
            ...
            if (hasHiragana && !hasKanji) {
                return node::WordType::HIRAGANA;
            } else if (!hasHiragana && hasKanji) {
                return node::WordType::KANJI;
            } else if (hasHiragana && hasKanji) {
                return node::WordType::MIXED;
            } else {
                return node::WordType::UNKNOWN;
            }
        }
