// NgramCoreLib.cpp : スタティック ライブラリ用の関数を定義します。
//

#include "NgramCoreLib.h"
...
namespace NgramCoreLib {
    ...
    int NgramMakeChar4gram(const wchar_t* inputPath, const wchar_t* outputPath, const wchar_t* logFilePath, wchar_t* errMsgBuf, size_t bufsiz, bool showError) {
        ...
        try {
            if (!inputPath || !*inputPath || !outputPath || !*outputPath) {
                THROW_RTE(L"make-char-4gram requires input and output paths");
            }
            dict::Char4gram::build(inputPath, outputPath);
        } catch (RuntimeException ex) {
            ERROR_HANDLER->Error(ex.getMessage());
            if (showError) printError(ex);
        } catch (...) {
    ...
    int NgramAnalyze(const wchar_t* sentence, const wchar_t* tempEntries, wchar_t* wakati_buf, size_t bufsize, bool bStdout, wchar_t* errMsgBuf, size_t bufsiz) {
        LOG_INFOH(L"ENTER: sentence={}, tempEntries={}", sentence ? sentence : L"null", tempEntries ? tempEntries : L"null");
        ERROR_HANDLER->Clear();
