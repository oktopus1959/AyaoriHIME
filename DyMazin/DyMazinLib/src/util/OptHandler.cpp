...
namespace {
    Vector<Vector<String>> defaultOptions = {
        ...
        Vector<String>({L"m/model:FILE",                   L"", L"use FILE as model file"}),
        Vector<String>({L"u/userdic:FILE",                 L"", L"use user dictionary"}),
        Vector<String>({L"l/lattice-level:INT",           L"0", L"lattice information level (DEPRECATED)"}),
        Vector<String>({L"D/dictionary-info",              L"", L"show dictionary information and exit"}),
        Vector<String>({L"O/output-format-type:TYPE",      L"", L"set output format type (wakati,none,...)"}),
        Vector<String>({L"A/all-morphs",                   L"", L"output all morphs(default false)"}),
        Vector<String>({L"N/nbest:INT",                   L"1", L"output N best results (default 1)"}),
        Vector<String>({L"?/partial",                      L"", L"partial parsing mode (default false)"}),
        Vector<String>({L"?/marginal",                     L"", L"output marginal probability (default false)"}),
        Vector<String>({L"M/max-grouping-size:INT",      L"24", L"maximum grouping size for unknown words (default 24)"}),
        Vector<String>({L"a/assign-user-dictionary-costs", L"", L"only assign costs/ids to user dictionary"}),
        Vector<String>({L"n/build-unknown",                L"", L"build parameters for unknown words"}),
        Vector<String>({L"G/build-model",                  L"", L"build model file"}),
        Vector<String>({L"g/build-charcategory",           L"", L"build character category maps"}),
        Vector<String>({L"s/build-sysdic",                 L"", L"build system dictionary"}),
        Vector<String>({L"i/build-matrix",                 L"", L"build connection matrix"}),
        Vector<String>({L"w/wakati",                       L"", L"build wakati-gaki only dictionary"}),
        Vector<String>({L"p/posid",                        L"", L"assign Part-of-speech id"}),
        Vector<String>({L"F/node-format:STR",    L"%m\\t%H\\n", L"use STR as the user defined node format"}),
        Vector<String>({L"U/unk-format:STR",     L"%m\\t%H\\n", L"use STR as the user-defined unknown node format"}),
        Vector<String>({L"B/bos-format:STR",               L"", L"use STR as the user-defined beginning-of-sentence format"}),
        Vector<String>({L"E/eos-format:STR",         L"EOS\\n", L"use STR as the user-defined end-of-sentence format"}),
        Vector<String>({L"S/eon-format:STR",               L"", L"use STR as the user-defined end-of-NBest format"}),
        Vector<String>({L"x/unk-feature:STR",              L"", L"use STR as the feature for unknown word"}),
        Vector<String>({L"b/input-buffer-size:INT",        L"", L"set input buffer size (default 8192)"}),
        Vector<String>({L"P/dump-config",                  L"", L"dump config parameters"}),
        Vector<String>({L"C/allocate-sentence",            L"", L"allocate new memory for input sentence"}),
...
//#define ERROR_NOT_FOUND(x) LOG_ERROR(L"`%s`: not found", x.c_str()); result = false
    ...
    SharedPtr<OptHandler> OptHandler::CreateOptHandler(size_t argc, const wchar_t** argv, const wchar_t* logFilePath) {
        ...
        if (Reporting::Logger::LogFilename().empty() && logFilePath) {
            //Reporting::Logger::LogFilename = _T("DyMazinLib.log");
            Reporting::Logger::SetLogFilename(logFilePath);
            LOG_INFOH(L"\n==== START DyMazinLib: progname={}, args={} ====\n", progname, utils::join(args, L" | "));
        }
