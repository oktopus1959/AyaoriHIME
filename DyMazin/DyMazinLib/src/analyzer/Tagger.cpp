...
namespace analyzer {
     ...
     * @return 解析結果
     */
    String Tagger::parse(StringRef sentence, size_t nBest, int mazePenalty, int mazeConnPenalty, bool allowNonTerminal) {
        LOG_INFO(L"ENTER: sentence={}", sentence);
        Vector<String> results;
        parseNBest(sentence, results, nBest, mazePenalty, mazeConnPenalty, allowNonTerminal);
     ...
     * @return 最良解析結果のコスト
     */
    int Tagger::parseNBest(StringRef sentence, Vector<String>& results, size_t nBest, int mazePenalty, int mazeConnPenalty, bool allowNonTerminal) {
        //warnings.clear();
