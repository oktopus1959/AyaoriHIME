...
|LatticeImpl.clear\w*|kanjiPref=true|rollOver=\w+|Non rollover.*penalty=[0-9]+|commitOnlyWithSameLeaderString.*|LOCAL.lattice.addCandidate| totalCost=[0-9]+|newCandStr=|.totalCost=.*isStrokeBS=.*\
|LOCAL.lattice.updateRealtimeNgram|LatticeImpl.addPieces.*(ENTER|LEAVE)|: emptyResult|pieces is empty .*' | \
