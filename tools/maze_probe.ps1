...
public static class MazeProbeNative {
...
"@
    ...
    function New-OutParams {
        $o = New-Object MazeProbeNative+DecoderOutParams
    ...
    function New-CmdPtr([string]$text) {
        $p = New-Object MazeProbeNative+DecoderCommandParams
        $p.inOutData = New-Object 'char[]' 2048
        if ($text) {
            $chars = $text.ToCharArray()
            [Array]::Copy($chars, 0, $p.inOutData, 0, [Math]::Min($chars.Length, $p.inOutData.Length - 1))
        }
        $size = [Runtime.InteropServices.Marshal]::SizeOf([type][MazeProbeNative+DecoderCommandParams])
    ...
    function New-DeckeyPtr([string]$text) {
        $p = New-Object MazeProbeNative+DecoderHandleDeckeyParams
        $p.editBufferData = New-Object 'char[]' 64
        if ($text) {
            $chars = $text.ToCharArray()
            [Array]::Copy($chars, 0, $p.editBufferData, 0, [Math]::Min($chars.Length, $p.editBufferData.Length - 1))
        }
        $size = [Runtime.InteropServices.Marshal]::SizeOf([type][MazeProbeNative+DecoderHandleDeckeyParams])
        $ptr = [Runtime.InteropServices.Marshal]::AllocCoTaskMem($size)
        [Runtime.InteropServices.Marshal]::StructureToPtr($p, $ptr, $false)
        return $ptr
    }
    ...
    function Send-Cmd([IntPtr]$decoder, [string]$cmd, [ref]$outRef) {
        $ptr = New-CmdPtr $cmd
        try {
            [void][MazeProbeNative]::ExecCmdDecoder($decoder, $ptr, $outRef)
        } finally {
    ...
    $out = New-OutParams
    $decoder = [MazeProbeNative]::CreateDecoder($LogLevel)
    ...
    try {
        $ptr = New-CmdPtr ""
        try {
            [void][MazeProbeNative]::InitializeDecoder($decoder, $ptr)
        } finally {
            [Runtime.InteropServices.Marshal]::FreeCoTaskMem($ptr)
        }
        ...
        foreach ($deckey in $DeckeySequence) {
            ...
            try {
                [void][MazeProbeNative]::HandleDeckeyDecoder($decoder, $ptr, $deckey, 0, 0, [ref]$out)
            } finally {
                [Runtime.InteropServices.Marshal]::FreeCoTaskMem($ptr)
            }
        ...
        $mazeEdit = $PrefixEdit + $buffer
        $out = New-OutParams
        $ptr = New-DeckeyPtr $mazeEdit
        try {
            [void][MazeProbeNative]::HandleDeckeyDecoder($decoder, $ptr, $MazeDeckey, 0, 0, [ref]$out)
        } finally {
            [Runtime.InteropServices.Marshal]::FreeCoTaskMem($ptr)
        }

        $mazeOut = CharsToString $out.outString
        Write-Output ("MAZE key={0} edit='{1}' out='{2}' numBS={3}" -f $MazeDeckey, $mazeEdit, $mazeOut, [int]$out.numBackSpaces)
        ...
        [void][MazeProbeNative]::FinalizeDecoder($decoder)
    }
}
finally {
    Pop-Location
