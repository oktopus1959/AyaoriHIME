...
本書は AyaoriHIME のインストール、起動および終了について説明したものです。
...
AyaoriHIME を利用される場合は、上記注意をご理解いただき、
末尾の「[利用条件と免責](#利用条件と免責)」に同意の上でのご利用をお願いします。

## インストールとアンインストール
### インストール
[Releaseページ](https://github.com/oktopus1959/AyaoriHIME/releases/)
から最新の AyaoriHIME-v*.zip をダウンロードして、適当なフォルダに展開してください。
...
## 起動
bin\AyaoriHIME.exe を実行します。CMDプロンプトから実行してもよいですし、
...
設定項目を変更すると「適用」ボタンが有効になり、これをクリックすることで変更が反映されます。

テーブルファイル(配列定義ファイル)を修正した場合は、「再読込」ボタンをクリックしてください。
テーブルファイルが再読み込みされます。他にも「容易打鍵文字ファイル」、「打鍵ヘルプファイル」を修正した場合も「再読込み」をクリックしてください。

「部首合成辞書ファイル」や「部首連想辞書ファイル」を修正した場合は、「再読込」では反映されません。タスクトレイアイコンを右クリックして「再起動」を行ってください。
...
## 「漢直WS」からの移行
漢直WSを利用されている方は、`kanchoku.user.ini` を `AyaoriHIME` ルートフォルダにコピーしてから、「再読込」をクリックしてください。漢直WSの設定のうち、AyaoriHIMEでも有効なものを引き継ぐことができます。
...
また、不要なキー設定は、BS や Delete で削除してください。
...
## アップデート
新しい AyaoriHIME をダウンロードして展開したら、古い AyaoriHIME の `userFiles` フォルダ内のファイルを新しいほうにコピーしてください。
具体的には下図のファイルをコピーすれば良いでしょう。

配列の定義ファイルなど、他にも追加・修正しているファイルがあれば、それもコピーしてください。
ただし、`kwhist.roman.txt` のように AyaoriHIME があらかじめ提供しているファイルを修正している場合は、
