...
- [IME ON（あ）/ OFF（Ａ） キーを使いたい](#IME-ON（あ）-OFF（Ａ）-キーを使いたい)
- [英数字配列の選択](#英数字配列の選択)
    - [独自の英数字配列を使いたい](#独自の英数字配列を使いたい)
    - [英数字配列の仮想鍵盤を表示したい](#英数字配列の仮想鍵盤を表示したい)
- [漢織媛を一時的に停止したい](#漢織媛を一時的に停止したい)
    - [マウスを使う方法](#マウスを使う方法)
    - [Pauseキーで一時停止・再開する](#Pauseキーで一時停止再開する)
- [入力文字列の表示方法](#入力文字列の表示方法)
    - [TSF DLL のインストール](#TSF-DLL-のインストール)
    - [TSF DLL のアンインストール](#TSF-DLL-のアンインストール)
- [仮想鍵盤](#仮想鍵盤)
    - [表示のずれを直したい](#表示のずれを直したい)
    - [仮想鍵盤がじゃまです](#仮想鍵盤がじゃまです)
    - [仮想鍵盤が変な表示になりました](#仮想鍵盤が変な表示になりました)
...
- [IME連携](#IME連携)
    ...
    - [IME ではなく、アプリが文字を横取りしてしまう](#IME-ではなくアプリが文字を横取りしてしまう)
- [同時打鍵](#同時打鍵)
...
|QWERTY [JP Eisu Disabled]|「英数」キーを無効化して、CapsLockとして機能させる設定|
...
キーボードファイルは、拡張子を `.key` という名前にして、漢織媛のルートフォルダ配下の `tables/_keyboard` に作成します。 
...
## IME ON（あ）/ OFF（Ａ） キーを使いたい
最近のキーボードだと、「無変換」「変換」「ひらがな」のかわりに 「IME OFF(A)」と「IME ON(あ)」
キーが載っている場合もあるようです。

このキーを漢織媛における「無変換」や「変換」キーの代替として使いたい場合は、キーボードファイル (106.keyなど)
に下記を追記してください。
...
英数字テーブルファイルは、拡張子を `chars.*.txt` という名前にして、漢織媛のルートフォルダ配下の
...
実際に漢織媛が使用する配列定義は、`tmp/chars.current.txt` に出力されたものになります。
...
`Pause` キーを押すたびに漢織媛を一時停止・再開できるようになります。

![Suspend By Puse](image/suspend-by-pause.png)

## 入力文字列の表示方法
入力中の文字列は以下の3通りの方法で表示することができます。

- TSFを利用して、対象アプリに表示させる
- 漢織媛独自の編集ウィンドウに表示する
- 入力された文字列を都度システムに送出することにより、対象アプリに表示させる

これらの方法は、下記の「入力文字列の表示方法」設定で選択することができます。

![alt text](image/input-string-disp-methods.png)

配列融合を利用せずに単一の配列だけを使用する場合は、「入力された文字列を都度システムに送出する」方法が一番シンプルです。この方法であれば、入力文字列を MS-IME などにもそのまま渡すことができます。
この方法を使用する場合は、両方のチェックを外してください。

通常の IME を使用せず、ひらがなと漢直の両方の配列を融合させて使いたいという場合は、まず TSF の利用を検討してください。

TSF を利用する方法は、一般のIMEと同様の表示方式です。入力中の文字列には下線が引かれ、まだ対象アプリに送出されていない(未確定状態である)ことが判別できるようになっています。この方法を利用するには、TSF による DLL をインストールする必要があります。(インストールには管理者権限が必要となります)

漢織媛独自の編集ウィンドウを使用する場合は、DLLのインストールが不要です。管理者権限のないPCで使用する場合はこちらをご使用ください。編集ウィンドウは、対象アプリのテキストカーソル位置に表示するようにしていますが、アプリによってはカーソル位置を正しく返さないものがあります。というか、最近のアプリはほとんどがそうだと言ってもよいかもしれません。

編集ウィンドウがあまりにもカーソル位置と離れたところに表示されるような場合は、このチェックをOFFにしてやると、文字が入力される都度、それを対象アプリに送出するようになります。これならばカーソル位置に入力文字列が表示されることが保証されますが、入力文字候補の入れ替えが発生した時に、表示がチラチラ、バタバタするというデメリットもあります。
...
コンソールが開いたら、AyaoriHIME を展開したディレクトリ配下の `bin` に移動します。

そこで下記バッチを実行してください。

```
register_tsf_dll.bat
```

下図のように登録ポップアップが表示されます。このポップアップは、32bit版と64bit版の2回表示されます。

![alt text](image/register-tsf-dll.png)

DLLのインストールが完了したら、画面右下の言語選択で `AyaoriHIME TSF Output` を選択してください。
なお、環境によっては `漢` アイコンではなく `日本` になっている場合があります。

![alt text](image/言語選択.png)

### TSF DLL のアンインストール
インストール時と同様に CMD プロンプトを開いて、次のバッチを実行してください。

```
unregister_tsf_dll.bat
```

## 仮想鍵盤
漢織媛を起動し、漢字かな入力モードに入ると、初期状態ではテキストカーソルの近くに下図のような
「仮想鍵盤」が表示されます。
...
### IME に送るローマ字定義を変更したい
IME をローマ字入力モードで使っている場合は、漢織媛でかなが入力されると、それに対応してデフォルトで設定してあるローマ字がIMEに対して送られます。
この「かな→ローマ字」変後定義を独自の設定にしたい場合は、漢織媛のルートフォルダにある `kana-roman.def.sample.txt` を `kana-roman.def.txt` という名前でコピーし、その内容を編集してください。
...
修正したファイルを保存した後、漢織媛の設定ダイアログなどで設定の「再読込」を実行してください。

### IME ではなく、アプリが文字を横取りしてしまう
IME がONになっている対象アプリによっては、漢字やカタカナをそのまま送るとIMEではなくアプリが文字を横取りしてしまう場合があります。そのような場合は、「IMEの入力モード対応時にカタカナをひらがなに変換」にチェックを入れてください。
...
W を押している時間帯と J を押している時間帯が一瞬でも重なってる必要があります。<br>
(以下の図では、左から右に時間が流れていると思ってください)

![Wj Combo](image/wj-combo.png)

漢織媛 では、この「同時」の時間帯が無い場合は、どんなに近接した打鍵であっても「同時打鍵」とはみなしません。
...
という条件を設定することができます。
[設定ダイアログ](#設定ダイアログ)の
「同時打鍵・IME」>「同時打鍵の判定条件」>「A: 第1キー押下から第2押下までの時間」で設定してください。
...
「40ms」という制約によって適用されなくなり、左シフトは第3打鍵(K)にかかるようになります。
...
上図の「第2打鍵以降のキー押下でタイマーを併用する」にチェックを入れてください。
...
下図のように設定ダイアログの「修飾キー」タブ「拡張修飾キー」設定で、 `KatakanaModeToggle` に適当なキーを設定してください。<br>
![alt text](image/katakan-mode-toggle.png)

設定の方法については [FAQ-キーアサイン編](FAQ-キーアサイン.md) を参照してください。

### ひらがなを出力した後、一括でカタカナに変換する
いったんひらがなを出力した後、それを一括でカタカナに変換します。
ただし出力文字列の末尾のひらがなを全てカタカナに変換してしまうので注意が必要です。

カタカナにしたくないひらがながある場合は、そこでスペースを入れておくとよいでしょう。
スペース以降のひらがなをカタカナに変換後、スペースを削除します。

```
私は ばか → 私はバカ
```

末尾がカタカナの場合は、末尾のカタカナ列の先頭文字のみをひらがなに戻すので、
望みの文字列だけがカタカナになるまで、この機能を呼び出してもよいでしょう。

```
私はばか → 私ハバカ → 私はバカ
```

こちらも変換のためのキーの設定が必要になります。上図で `OneshotKatakanaConversion`
に適当なキーを設定してください。

### 英数モードで英単語またはローマ字からカタカナに変換
「[英数モード](#英数モード)」で入力中に簡易辞書検索を呼び出すと、入力した英字列で簡易辞書の検索を行い、
マッチするものがあれば定義されているカタカナに変換します。

<p style="padding-left:2em;"><font size="-1">「簡易辞書・英数字・カタカナ」>「英数字入力モード」>「□ が押されたら入力した英字列をカタカナに変換」
で指定したキーを入力することでも簡易辞書検索を呼び出せます。</font></p>

#### 英単語辞書による変換
たとえば、`Computer` と入力したところで簡易辞書検索を呼び出すと「コンピュータ」に変換されます。
これは、付属の `systemFiles/kwhist.system.roman.txt` というファイルに 「Computer|コンピュータ」というエントリが
登録されており、簡易辞書検索によって `|` の前半部にマッチしたらその後半部が出力されるためです。

<p style="padding-left:2em;"><font size="-1"><code>kwhist.system.roman.txt</code> には
<a href="https://clrd.ninjal.ac.jp/unidic/">UniDic</a> から抽出した3万8千語弱の単語が登録されています。</font></p>

もし望みの単語が登録されていない場合は、「[英字列からカタカナへの変換を定義する](#英字列からカタカナへの変換を定義する)」を参照して単語登録を行ってください。

「ファイルサーバ」のような複合語を一度に変換したい場合は、「FileServer」のように単語ごとに
先頭文字を大文字にしてください。
ただしこの場合は、簡易辞書検索でヒットした先頭候補がそのまま出力されるので、ご注意を。
望みのカタカナに変換されなかった場合は、いったんキャンセルして、単語ごとに検索してみてください。
個別に望みのカタカナを選択した後は、次からはその選択した語が先頭候補になります。

#### ローマ字変換
簡易辞書辞書にマッチするエントリがなかった場合は、英字列をローマ字変換します。
たとえば、「Gandam」と入力して簡易辞書検索を実行すると、「ガンダム」に変換されます。
ローマ字からカタカナへの変換規則は `kwroman.def.txt` というファイルで定義しているので、
このファイルを修正することで変換規則を自分用にカスタマイズすることもできます。

![alt text](image/roman-def-file.png)
...
<!--
#### カタカナ変換時に横列の候補表示をしない
デフォルトでは、簡易辞書検索を呼び出して英字列からカタカナ変換をすると下図のような候補列が表示されます。
...
## ギリシャ文字の出し方
英数モードで英大文字または英単語からギリシャ文字に変換できます。
「[英数モード](#英数モード)」で入力中に簡易辞書検索を呼び出すと、入力した英字列で簡易辞書の検索を行い、
マッチするものがあれば定義されているギリシャ文字(またはカタカナ)に変換します。

基本的に英文字1文字の場合は小文字のギリシャ文字に、フルスペルの場合はカタカナまたは大文字のギリシャ文字に変換します。
...
他候補がある場合は、デフォルトで `Ctrl-I` で次の候補が選択されます。

独自の候補を定義をしたい場合は、「[英数モード](#英数モード)」の設定で「英字列変換辞書ファイル」に定義を追加してください。

## 英数モード
「簡易辞書・英数字・カタカナ」>「英数入力モード」>「英大文字押下で英数モードに移行」にチェックを入れると、
...
#### 英字列をカタカナに変換する
詳細は「[英数モードで英単語またはローマ字からカタカナに変換](#英数モードで英単語またはローマ字からカタカナに変換)」を参照ください。
...
#### 英字列からカタカナへの変換を定義する
望みの単語が登録されていない場合は、「英字列変換定義ファイル」の「開く」ボタンをクリックして `userFiles/kwhist.roman.txt` を開き、「Gundam|ガンダム」というような文字列を追加してから、「再読込」ボタンをクリックしてください。
