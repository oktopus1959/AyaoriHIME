...
インストール直後には、最低限の設定が記述された ayaorihime.ini だけが存在しています。
KanchokuWS を使用されている場合は、その kanchoku.user.ini の設定を引き継ぐこともできます。

KanchokuWS の kanchoku.user.ini を使う場合は、それを AyaoriHIME のルートフォルダにコピーしてから
起動（または再起動）してください。
...
AyaoriHIME が使用するファイルは、設定ファイルを除き、すべて UTF-8 でエンコードされています。
