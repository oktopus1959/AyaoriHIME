...
「履歴検索・候補選択する」にチェックを入れて適当な Ctrl 修飾キーを設定すると、任意のタイミングで候補を検索できます。
また、「@」(T-Code/106.key;デフォルト) を押すと、後述の縦列鍵盤で候補を直接選択できます。
...
明示的な簡易辞書検索では、横列に表示できる候補数は最大10個です。
...
##### 削除と再登録
登録された変換形を削除する場合は、上述した履歴候補の削除機能を利用してください。
再度利用する場合は、上述の手動登録を実行してください。
