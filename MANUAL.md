...
- [仮想鍵盤](#仮想鍵盤)
    ...
    - [直前打鍵位置の表示](#直前打鍵位置の表示)
- [Ctrlキーや拡張修飾キーのリマップ](#Ctrlキーや拡張修飾キーのリマップ)
...
|okt3.tbl|OKT-Code|作者個人用(T-Codeから派生)。「のにいると」と一緒に使用する前提|
...
|successive|相互シフトかつ連続シフトあり（相互連続シフト; デフォルト）|
...
たとえば、 `userFiles/alt-kanji.txt` に
```
乗 剰
```
という代替文字定義があると、元のテーブル定義にあった「乗」が「乗|剰」に書き換えられて登録されます。この `|` による複数文字の定義については、[FAQ-配列融合編](FAQ/FAQ-配列融合.md) を参照してください。

代替文字では**裏文字**も定義できます。
裏文字は、代替文字の次の要素として記述します。たとえば、`userFiles/alt-kanji.txt` に次のような定義があると、空白を区切り文字として、「祈|匠」は代替文字、「斤」が裏文字となります。
```
近 祈|匠 斤
```

表文字と同一のストローク列に裏文字を定義するには、`#addAltKanji second` を使います。
ただ、そのままだと同一ストローク列に対して、表文字と裏文字が二重定義されてしまうので、
裏文字のストローク列は次のようにして何らかのプレフィックスを付けて定義します。
```
#addAltKanji second
-40>
...
`#addAltKanji` の使用例としては、 `漢直系/okt3.tbl` を参照ください。

#### 副テーブルファイル
テーブルファイルは2つまで設定することができます。ここでは、従来からあるものを「主テーブルファイル」と呼びます。
複数テーブルの使い方については、[FAQ-配列融合編](FAQ/FAQ-配列融合.md) を参照してください。

#### テーブルファイルの再読み込み
エディタなどでテーブルファイルを修正した後は、タスクトレイアイコンの右クリックメニューから「再読込」を
...
<!--
後述する簡易辞書や交ぜ書き入力において、容易打鍵文字だけを含むような候補を表示しないようにしたり、
...
簡易辞書検索、交ぜ書き変換、部首合成など、後述するいくつかの機能は、
...
簡易辞書の検索は、明示的な操作でのみ実行されます。登録は設定ダイアログ、選択文字列の登録、またはミニバッファへの貼り付けで行ってください。

「SimpleDicNext」または「SimpleDicPrev」を割り当てると、任意のタイミングで候補を検索して巡回できます。

候補表示中は、設定に応じて上下矢印キーまたは Tab / Shift+Tab キーで候補を移動できます。設定ダイアログの「Enterキーで先頭候補を選択する」が有効なら、Enter キーで先頭候補を出力できます。

#### ワイルドカードの利用

簡易辞書検索のキーには、ワイルドカード(`?` と `*`)を使うことができます。
`?` は任意の1文字に、`*`は0文字以上の任意の文字列にマッチします。
たとえば、後述の簡易辞書登録を利用して「新疆ウイグル自治区」という文字列を簡易辞書に登録しておくと、
「新?ウ*区」という入力がこれにマッチして、以下のような候補表示が得られます。

![SimpleDic Wildcard](image/hist-wildcard.png)

また、先頭文字が直接打鍵できないような熟語についても、
「?退」などと入力すると次のような簡易辞書候補を得ることができます。

![SimpleDic Wildcard2](image/hist-wildcard2.png)
...
- 設定ダイアログの「辞書登録」>「簡易辞書登録」で登録する

たとえば「お世話になっております」を簡易辞書へ登録するには、
...
### 変換形
手動による簡易辞書登録の応用として、「変換形」登録があります。これは、
```
キー文字列 | 変換形文字列
```
という形式で簡易辞書登録しておくと、選択された際にキー文字列を「変換形文字列」に置換して出力する、
というものです。
...
#### 交ぜ書き変換形の登録
交ぜ書き変換の結果を簡易辞書で利用したい場合は、「読み」と「変換形」を `|`
で結合した文字列を手動登録します。

たとえば、「み力」⇒「魅力」という変換を登録するには、`み力|魅力` を登録します。
「み力」と入力すると「み力|魅力」が候補として表示され、それを選択すると「魅力」に置換されます。

下記ツイートに交ぜ書き変換形の使用例を動画で投稿していますので、興味のある方は参照ください。<br/>
...
## 英数記号全角入力
全角変換モードと通常入力モードを切り替える機能があります。
モードを切り替えるにはまず全角モードと通常モードの切り替え機能を呼び出す機能キーの設定が必要です。
下図のように設定ダイアログの「修飾キー」>「拡張修飾キー」設定で「有効」にチェックを入れて「適用」した後、
「編集」ボタンをクリックしまず。<br>
![alt text](image/mod-conv-tab.png)

`ZenkakuModeToggle` を適当なキーに割り当ててください。<br>

![alt text](image/toggle-zenkaku-mode.png)

たとえば、`Insert` キーに割り当てるには、拡張修飾キー設定で次のように設定します。`Insert` のところでダブルクリックすると、上のダイアログが開くので、`ZenkakuModeToggle` を選択してください。<br>
![alt text](image/insert-to-toggle-zenkaku.png)
...
設定方法については、「[FAQ-基本編#カタカナの出し方](FAQ/FAQ-%E5%9F%BA%E6%9C%AC.md#%E3%82%AB%E3%82%BF%E3%82%AB%E3%83%8A%E3%81%AE%E5%87%BA%E3%81%97%E6%96%B9)」を参照してください
...
ただし、この設定をした場合は、AutoHotKey などによるキー変換の結果も当ツールによる処理の対象外になりますので、留意ください。
-->
...
- 形態素解析用の辞書類 (systemFiles/dymazin/dic/mazedic/配下) は NAIST 松本研で作成された mecab および mecab-ipadic を改編したものです。
  - mecab-ipadic のライセンスについては COPYING.mecab-ipadic を参照ください。
- kwhist.system.roman.txt は、国立国語研究所で作成された「UniDic」(https://clrd.ninjal.ac.jp/unidic/)
  から抽出したものに手を加えています。したがって当ファイルには UniDic と同じ CC BY-NC-SA ライセンスが適用されます
